...
public class StunClient3489 : IUdpStunClient
{
	...
	private async ValueTask<StunResponse?> RequestAsync(StunMessage5389 sendMessage, IPEndPoint remote, IPEndPoint receive, CancellationToken cancellationToken)
	{
		try
		{
			...
			Console.WriteLine($"send message: (transaction id: {PrintByteArray(sendMessage.TransactionId)}{PrintAttributes(sendMessage.Attributes)}) to {remote.Address}:{remote.Port} via {LocalEndPoint.Address}:{LocalEndPoint.Port}");
			...
			var parseResult = message.TryParse(buffer[..r.ReceivedBytes]);
			Console.WriteLine($"recv message: (transaction id: {PrintByteArray(sendMessage.TransactionId)}) from {r.RemoteEndPoint} via {LocalEndPoint.Address}:{LocalEndPoint.Port}");
			if (parseResult && message.IsSameTransaction(sendMessage))
			{
	...
	private string PrintAttributes(IEnumerable<StunAttribute> attributes)
	{
		var sb = new StringBuilder("");
		foreach (var attr in attributes)
		{
			sb.AppendFormat($", {attr.Type}({attr.Value.AsString()})");
		}
		return sb.ToString();
	}
